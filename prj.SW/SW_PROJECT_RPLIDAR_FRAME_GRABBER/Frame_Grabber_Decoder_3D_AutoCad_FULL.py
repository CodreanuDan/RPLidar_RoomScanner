...
class LIDARScannerGUI:
    ...
    def setup_gui(self):
        ...
        self.open_cad_btn = ttk.Button(proc_frame, text="Open CAD (DXF)", command=self.open_cad, state=tk.DISABLED)
        self.open_cad_btn.grid(row=0, column=4, padx=5)
    ...
    def process_worker(self, measurement_folder, frames_folder, angle_step):
        try:
            ...
            self.log("Exporting to CAD format...")
            cad_path = os.path.join(measurement_folder, "Lidar_Model.dxf")
            self.export_to_cad(df_filtered, cad_path)
            self.log(f"Saved CAD file: {cad_path}")
            ...
            self.root.after(0, lambda: self.open_cad_btn.config(state=tk.NORMAL))
    ...
    def decode_measurements(self, scan_data: bytes, turret_angle_deg: float) -> list[dict]:
        ...
        data = np.frombuffer(scan_data, dtype=np.uint8)
        n_points = len(data) // 5
        data = data[:n_points*5].reshape(-1, 5)
        
        b0, b1, b2, b3, b4 = data.T

        valid_mask = ((b0 >> 2) & 0x01 == 1) & ((b0 & 0x01) != ((b0 >> 1) & 0x01))
        
        distance_mm = ((b4.astype(np.uint32) << 8) | b3) / 4.0
        valid_mask &= (distance_mm > 0) & (distance_mm <= 12000)

        if not np.any(valid_mask):
            return []

        d_mm = distance_mm[valid_mask]
        q = (b0[valid_mask] >> 3).astype(int)
        angle_deg = (((b2[valid_mask].astype(np.uint32) << 7) + (b1[valid_mask] & 0x7F)) / 64.0) % 360.0

        lidar_rad = np.deg2rad(angle_deg)
        turret_rad = np.deg2rad(turret_angle_deg)

        r_in_plane = d_mm * np.sin(lidar_rad)
        
        df = pd.DataFrame({
            "Quality": q,
            "LIDAR_Angle (deg)": np.round(angle_deg, 1),
            "Distance (mm)": d_mm.astype(int),
            "Turret_Angle (deg)": turret_angle_deg,
            "X (mm)": np.round(r_in_plane * np.cos(turret_rad), 2),
            "Y (mm)": np.round(r_in_plane * np.sin(turret_rad), 2),
            "Z (mm)": np.round(d_mm * np.cos(lidar_rad), 2)
        })
    ...
    def export_to_cad(self, df, output_path):
        try:
            self.log(f"Generare fișier CAD: {os.path.basename(output_path)}...")
            
            # Creem un desen nou compatibil cu versiuni vechi de AutoCAD (R2010)
            doc = ezdxf.new('R2010')
            msp = doc.modelspace()
            ...
            for p in points:
                # p este un array de tip [x, y, z]
                msp.add_point(p)
                
            doc.saveas(output_path)
            self.log(f"Succes! Fișier CAD salvat: {output_path}")
            
        except Exception as e:
            self.log(f"EROARE la exportul CAD: {e}")
    ...
    def open_cad(self):
        if self.current_measurement_folder:
            cad_path = os.path.join(self.current_measurement_folder, "Lidar_Model.dxf")
            if os.path.exists(cad_path):
                os.startfile(cad_path)
                self.log(f"Opened: {cad_path}")
            else:
                messagebox.showwarning("Warning", "CAD file not found. Process frames first.")
