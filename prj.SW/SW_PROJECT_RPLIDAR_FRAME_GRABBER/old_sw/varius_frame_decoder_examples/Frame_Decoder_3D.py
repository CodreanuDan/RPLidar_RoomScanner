...
def get_valid_measurement_from_sample_data(scan_data: bytes, turret_angle_deg: float) -> List[Dict]:
    ...
    for i in range(0, len(scan_data), 5):
        ...
        # - Each frame is a VERTICAL SLICE through the room
        # - LIDAR angle (0-360°) determines position within that vertical slice
        # - Turret rotates horizontally (0-180°), sweeping the slice around
        
        # In the vertical slice plane:
        # - One component is "depth" into the room (call it 'r')
        # - Other component is "height" (Z)
        
        # LIDAR angle maps to position in vertical plane:
        depth_in_slice = distance_mm * math.cos(lidar_angle_rad)  # radial depth
        z_global = distance_mm * math.sin(lidar_angle_rad)         # height
        
        # Turret angle rotates the depth component horizontally into X, Y:
        x_global = depth_in_slice * math.cos(turret_rad)
        y_global = depth_in_slice * math.sin(turret_rad)
