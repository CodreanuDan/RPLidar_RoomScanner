...
def get_valid_measurement_from_sample_data(scan_data: bytes, turret_angle_deg: float) -> List[Dict]:
    ...
    for i in range(0, len(scan_data), 5):
        ...
        # SIMPLE STACKING MODE - NO ROTATION
        # Just stack each frame vertically to see what each frame looks like
        # This helps us understand the actual scan pattern
        
        # Treat each frame as a 2D slice in the X-Y plane
        # LIDAR angle determines X, Y position (the 2D slice)
        x_global = distance_mm * math.cos(lidar_angle_rad)
        y_global = distance_mm * math.sin(lidar_angle_rad)
        
        # Stack frames vertically - each frame gets a different Z based on turret angle
        # This creates a "sandwich" of slices
        z_global = turret_angle_deg * 15  # 15mm per degree = ~2700mm total height for 180 frames
